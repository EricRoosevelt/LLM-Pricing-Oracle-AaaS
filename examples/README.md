...
1. Agent 先向控制面请求 `decision`
2. Agent 拿着返回的候选梯队，自己用厂商 API Key 直接调用大模型
3. 如果第一名模型宕机、超时或限流，自动切到下一名继续请求
4. Agent 最后把执行结果回写到 `/v1/routing/outcomes`
...
ORACLE_API_KEY=replace-with-your-control-plane-key
...
- 如果你的 Agent 框架支持工具调用，可以把 `create_decision`、`invoke_with_fallback` 和 `report_outcome` 直接封装成工具函数
